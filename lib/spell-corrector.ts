...
export const SCIENTIFIC_SYNONYMS: Record<string, string> = {
  // Herb common name → canonical botanical name (properly capitalized per botanical nomenclature)
...
export function correctAndNormalize(input: string): CorrectionResult {
  ...
  const synonymApplied = spellCorrected !== synonymNormalized; // Case-sensitive: detects case-only changes too (e.g. "curcuma longa" → "Curcuma longa")
