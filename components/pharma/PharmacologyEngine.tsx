...
export default function PharmacologyEngine({ onSearch, onSignInRequired }: PharmacologyEngineProps) {
  ...
  const { loading: pdfLoading, downloadPharmacologyPdf } = usePdfExport();
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-8">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs md:text-sm">Enter a natural product to explore its pharmacological profile and phytochemical composition from PubMed</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-6">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              <div>
                ...
                <p className="text-slate-400 text-xs md:text-sm">Pharmacological profile — PubMed literature analysis</p>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              ...
              <button
                onClick={() => downloadPharmacologyPdf({
                  herb: result.herb,
                  pharmacologicalActions: result.pharmacological_actions as unknown as Record<string, unknown>[],
                  activeCompounds: result.active_compounds as unknown as Record<string, unknown>[],
                ...
                disabled={pdfLoading}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-300 hover:border-gray-900 disabled:border-gray-300 disabled:opacity-60 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-all shadow-sm"
              >
                {pdfLoading ? (
            ...
            {summary && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 md:p-4 mb-4">
                ...
                <div className="text-emerald-900 text-xs md:text-sm leading-relaxed whitespace-pre-line">{summary}</div>
              </div>
            )}
            ...
            {result.active_compounds.length > 0 && result.evidence_level !== 'No Evidence' && (
              <div className="mb-4">
                <h4 className="text-xs md:text-sm font-extrabold text-slate-800 uppercase tracking-wider mb-3">Active Compounds</h4>
                <div className="flex flex-wrap gap-2">
            ...
            {result.evidence_level !== 'No Evidence' && actions.length > 0 && (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 md:p-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-bold text-emerald-800 text-sm md:text-base">Pharmacological Actions & Mechanisms</span>
                </div>

                <div className="space-y-3 max-h-[24rem] md:max-h-96 overflow-y-auto">
                  {actions.map((action, index) => (
                    <div key={`${action.name}-${index}`} className="bg-white border border-emerald-200 rounded-xl p-3">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        <span className="px-2 md:px-2.5 py-1 bg-emerald-100 border border-emerald-200 rounded-lg text-xs md:text-sm text-emerald-800 font-medium capitalize">{action.name}</span>
                        {action.score > 0 && (
