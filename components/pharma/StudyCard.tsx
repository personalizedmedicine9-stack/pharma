    ...
    >
      <div className="p-5 md:p-6">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {study.mechanismDescription && study.mechanismDescription !== 'Mechanistic pathway not fully characterized from available evidence' && (
              <span className="inline-block mt-1 px-2 py-0.5 bg-sky-50 border border-sky-200 rounded text-[11px] text-sky-700 font-semibold uppercase tracking-wide">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          <span className="px-2 py-0.5 rounded-full text-[11px] md:text-xs font-bold bg-blue-600 text-white">
            PubMed #{study.pmid}
          </span>
          <span className={`px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold ${RELEVANCE_STYLES[study.relevanceLabel]}`}>
            {study.relevanceLabel} Relevance
          </span>
          <span className="px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold bg-slate-800 text-white">
            {STUDY_TYPE_ABBR[study.studyType] ?? '??'}&nbsp;{study.studyType}
          </span>
          <span className={`px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold ${EVIDENCE_STYLES[study.evidenceLevel]}`}>
            {study.evidenceLevel} Evidence
          </span>
          <span className={`px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold ${JOURNAL_STYLES[study.journalQuality]}`}>
            {study.journalQuality}
          </span>
          {study.citationCount > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold bg-teal-100 text-teal-800 border border-teal-200">
              <TrendingUp size={10} />
              {study.citationCount.toLocaleString()} citations
            </span>
          )}
          <span className="px-2 py-0.5 rounded-full text-[11px] md:text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">
            Score {study.compositeScore}
          </span>
        </div>

        <div className="flex flex-wrap items-center gap-x-3 md:gap-x-4 gap-y-1 text-xs text-slate-500 mb-3">
      ...
      <div className="px-4 md:px-6 pb-3 border-t border-slate-50">
        <p className="text-[11px] md:text-xs text-slate-300 font-medium pt-2">Retrieved: {new Date().toLocaleDateString()}</p>
      </div>
        ...
        {abstractOpen && study.abstract && (
          ...
          >
            <div className="px-5 md:px-6 pb-5 md:pb-6 border-t border-slate-100 bg-slate-50/50">
              ...
              <p className="text-slate-600 text-[15px] leading-relaxed">{study.abstract}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
