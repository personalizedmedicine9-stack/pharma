        ...
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
          ...
          <div className="flex flex-wrap gap-1.5">
            {sourcesUsed.map((source) => (
              <span key={source} className="px-2 py-0.5 bg-white border border-gray-200 text-xs font-semibold rounded-md text-gray-600">
                {source}
              </span>
            ))}
          </div>
      ...
      {fdaData && (
        <div className="mt-4 bg-amber-50 border border-amber-200 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-800 mt-1">Potential interaction-related safety signals were identified from FDA-associated pharmacovigilance data; however, clinical significance and causality remain incompletely established.</p>
          </div>
        </div>
      )}
    </div>
