...
export default function InteractionEngine({ history, onHistoryUpdate, onSignInRequired }: InteractionEngineProps) {
  ...
  const { loading: pdfLoading, downloadInteractionPdf } = usePdfExport();
  ...
  const [aiStructured, setAiStructured] = useState<Record<string, string> | null>(null);
  ...
  const handleSearch = async (drugName: string, herbName: string) => {
    ...
    try {
      ...
      // Apply canonical names from spelling corrections for proper display/PDF
      if (data.spellingCorrections) {
        setSpellingCorrections(data.spellingCorrections);
        if (data.spellingCorrections.drug?.canonical || data.spellingCorrections.drug?.corrected) {
          drugName = data.spellingCorrections.drug.canonical || data.spellingCorrections.drug.corrected;
        }
        if (data.spellingCorrections.herb?.canonical || data.spellingCorrections.herb?.corrected) {
          herbName = data.spellingCorrections.herb.canonical || data.spellingCorrections.herb.corrected;
        }
      }

      // Store canonical names for PDF title and display
  ...
  const handleSummary = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.text) {
        if (data.structured && typeof data.text === 'object') {
          // Structured AI response — store both the structured data and a plain text summary
          setAiStructured(data.text as Record<string, string>);
          setSummary(data.text.executiveSummary || data.text.evidenceSynthesis || null);
        } else {
          setSummary(typeof data.text === 'string' ? data.text : JSON.stringify(data.text));
        }
      }
  ...
  return (
    <div>
      {/* Search form section */}
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-4 md:p-8 mb-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
          {API_SOURCES.map((s) => (
            <div key={s.name} className="bg-gray-50/80 border border-gray-100 p-3 md:p-4 rounded-xl">
              <div className="flex items-center gap-2 mb-1">
                <div className={`w-2 h-2 rounded-full ${s.color}`}></div>
                <span className="text-xs md:text-sm font-bold text-gray-900">{s.name}</span>
              </div>
              <p className="text-[10px] md:text-xs text-gray-500 pl-4 font-medium">{s.desc}</p>
            </div>
          ))}
        </div>
          ...
          <div className="mb-6 p-4 bg-sky-50 border border-sky-200 rounded-xl">
            ...
            <div className="flex flex-wrap gap-2">
              {uniqueMechanisms.map((m, i) => (
                <span key={i} className="px-3 py-1.5 bg-sky-100 border border-sky-200 rounded-lg text-xs md:text-sm text-sky-800 font-medium">{m}</span>
              ))}
            </div>
          </div>
        );
        ...
        {hasSearched && results.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              <button
                onClick={() => downloadInteractionPdf({
                  results: results as unknown as Record<string, unknown>[],
                  drug: lastDrug,
                  herb: lastHerb,
                  sourcesUsed,
                  fdaData: fdaData as unknown as Record<string, unknown> | null,
                  topCitationCount,
                  aiSummary: summary,
                  aiStructured: aiStructured,
                  scores: evidenceScores as unknown as Record<string, unknown>[],
                  confidenceReasoning,
                })}
                disabled={pdfLoading}
                className="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 bg-white border border-gray-300 hover:border-gray-900 disabled:border-gray-300 disabled:opacity-60 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-all shadow-sm"
              >
                {pdfLoading ? (
                  <><div className="w-4 h-4 border-2 border-gray-500 border-t-transparent rounded-full animate-spin" /> Generating PDF…</>
                ) : (
            ...
            <AnimatePresence>
              {summary && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-rose-50 border border-rose-200 rounded-xl shadow-sm mb-6 overflow-hidden">
                  <div className="px-4 md:px-6 pt-5 pb-3 flex items-center gap-2 border-b border-rose-200">
                    <Sparkles size={18} className="text-rose-700" />
                    <h4 className="text-sm md:text-base font-extrabold text-gray-900 uppercase tracking-wider">Interaction Findings Summary</h4>
                  </div>
                  <div className="px-4 md:px-6 py-4 space-y-4">
                    {(() => {
                      ...
                      return (
                        <>
                          <div className="text-black text-xs md:text-sm leading-relaxed font-medium">{synthesisText}</div>
                          {profileText && (
                            <div className="pt-3 border-t border-rose-200">
                              ...
                              <div className="text-black text-xs leading-relaxed whitespace-pre-line font-mono">{profileText}</div>
                            </div>
                          )}
                        </>
                      );
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-4 md:p-8">
              ...
              {confidenceReasoning && (
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                  ...
                  <p className="text-xs md:text-sm text-blue-900 leading-relaxed analysis-text">{confidenceReasoning}</p>
                </div>
              )}
              ...
              {evidenceScores.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-xs md:text-sm font-extrabold text-gray-800 uppercase tracking-wider mb-3">Weighted Evidence Scoring</h3>
