...
/**
 * Major bioactive markers for herbs — compounds considered the principal
 * pharmacologically active constituents rather than minor/trace compounds.
 * Maps herb name (lowercase) to an array of compound base names.
 */
const MAJOR_BIOACTIVE_MARKERS: Record<string, string[]> = {
  'turmeric': ['curcumin', 'turmerone'],
  'curcuma longa': ['curcumin', 'turmerone'],
  'curcumin': ['curcumin'],
  ...
  'silybum marianum': ['silymarin', 'silibinin'],
  'ginkgo biloba': ['ginkgolides', 'bilobalide'],
  'ginkgo': ['ginkgolides', 'bilobalide'],
  'green tea': ['epigallocatechin gallate', 'l-theanine'],
  'camellia sinensis': ['epigallocatechin gallate', 'l-theanine'],
  'berberine': ['berberine'],
  'berberis vulgaris': ['berberine'],
  'goldenseal': ['berberine'],
  'ashwagandha': ['withanolides', 'withaferin a'],
  'withania somnifera': ['withanolides', 'withaferin a'],
  'kava': ['kavalactones'],
  'piper methysticum': ['kavalactones'],
  'licorice': ['glycyrrhizin', 'glabridin'],
...
/**
 * Check whether an action keyword and a mechanism keyword appear within
 * `maxDist` characters of each other in the same text. This prevents
 * unrelated co-occurrences (e.g., an article mentioning "antimicrobial" in
 * one paragraph and "acetylcholinesterase" in another) from being linked.
 */
function isProximate(text: string, actionRegex: RegExp, mechRegex: RegExp, maxDist = 150): boolean {
  // Find all positions where the action keyword matches
  ...
  while ((m = actionRe.exec(text)) !== null) {
    ...
    if (actionPositions.length > 50) break; // safety cap
  }
  if (actionPositions.length === 0) return false;

  // Find all positions where the mechanism keyword matches
  ...
  while ((m = mechRe.exec(text)) !== null) {
    ...
    if (mechPositions.length > 50) break; // safety cap
  }
  if (mechPositions.length === 0) return false;

  // Check if any action position is within maxDist of any mechanism position
...
function generatePharmacologyReasoning(
  ...
  // Compound profile
  if (compoundCount > 0) {
    const majorCount = compounds.filter(c => c.isMajor).length;
    if (majorCount > 0) {
      parts.push(`${compoundCount} bioactive compound${compoundCount > 1 ? 's' : ''} identified (${majorCount} major bioactive marker${majorCount > 1 ? 's' : ''})`);
    } else {
      parts.push(`${compoundCount} bioactive compound${compoundCount > 1 ? 's' : ''} identified from literature`);
    }
  } else {
  ...
  // Confidence assessment with scientific caution
  if (confidence === 'Low') {
    if (highScore === 0 && modScore === 0) {
      parts.push('low confidence due to absence of well-supported pharmacological actions and predominance of preliminary evidence requiring further validation');
    } else {
      parts.push('low confidence due to limited high-quality evidence; conclusions remain provisional');
    }
  } else if (confidence === 'Moderate') {
    parts.push('moderate confidence with some well-supported actions; however, human clinical validation remains limited and generalizability is uncertain');
  } else {
    parts.push('relatively robust evidence base; however, heterogeneity in study designs and populations warrants consideration when interpreting clinical applicability');
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const articles: { pmid: string; title: string; abstract: string; journal: string; doi?: string; authors: string[]; pubYear: string }[] = [];
    ...
    while ((match = articleRegex.exec(xml)) !== null) {
      ...
      // Extract authors from <AuthorList><Author><LastName>, <ForeName>
    ...
    // Active compounds — flag major bioactive markers
    const herbLower = (herbName || '').toLowerCase();
    const majorMarkersForHerb = new Set<string>();
    for (const [herbKey, markers] of Object.entries(MAJOR_BIOACTIVE_MARKERS)) {
      if (herbLower.includes(herbKey) || herbKey.includes(herbLower)) {
        markers.forEach(m => majorMarkersForHerb.add(m.toLowerCase()));
      }
    }
    ...
    const active_compounds: PharmacologyCompound[] = ACTIVE_COMPOUNDS
      ...
      .map(c => {
        ...
        // Check if this compound is a known major bioactive marker for the herb
        const compoundBaseName = (c.name || '').split('(')[0].trim().toLowerCase();
        const isMajor = majorMarkersForHerb.has(compoundBaseName);
        return { name: c.name, category: c.category, pmids: matched.slice(0, 3).map(a => a.pmid), ...(isMajor ? { isMajor: true } : {}) };
      })
    ...
    const actions: PharmacologyAction[] = PHARM_ACTIONS.map(action => {
      ...
      // A mechanism is only linked to this action if BOTH keywords appear
      // in the SAME article AND within 150 characters of each other.
      ...
      const mechanisms = MECH_KEYWORDS.map(m => {
        ...
        // First: articles that contain the mechanism keyword
        ...
        const proximate = mm.filter(a => {
          ...
          return isProximate(text, regex, mr, 150);
        });
      ...
      // If no mechanism found within proximity, add a default placeholder
